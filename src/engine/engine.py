...
from src.models.models import Command, RequestParam, ConnectorOutput, ResultOutput, DeviceOutput, CommandStatus
...
class Engine:
    ...
    def run(self) -> Tuple[dict, int]:
        ...
        device_error_mapping = {}
        
        driver_order = self.drivers.get_driver_order()
        
        for device in self.devices:
            print(f"Running device {device.get_ip()}")
            ...
            for driver_name in ordered_drivers:
                ...
                for connector_name in driver:
                    ...
                    count_os = sum(1 for command in commands if command.os == device.get_os())
                    print(f"Count commands for OS {device.get_os()} is {count_os}")
