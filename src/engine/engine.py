    ...
    def run(self) -> Tuple[dict, int]:
        ...
        for device in self.devices:
            ...
            for driver_name in ordered_drivers:
                ...
                if not success:
                    print(f"Failed to run any command for {device.get_ip()}")
                    device_output.stderr.append(CommandStatus(command_name='All', output='Failed to run any command', status='error'))
                    success = False
